...
def instantiate(
    ...
    if not multi_channel:
        return {
            "score_cam": lambda x: get_cam(x[:, 0:1, :, :].to(device), scam),
            "grad_cam": lambda x: get_cam(x[:, 0:1, :, :].to(device), gcam),
            "grad_cam_plus": lambda x: get_cam(x.to(device), gcam_plus),
            "abblation_cam": lambda x: get_cam(x[:, 0:1, :, :].to(device), acam)
        }
    else:
        return {
            "score_cam": lambda x: get_cam(x[:, :, :, :].to(device), scam),
            "grad_cam": lambda x: get_cam(x[:, :, :, :].to(device), gcam),
            "grad_cam_plus": lambda x: get_cam(x.to(device), gcam_plus),
            "abblation_cam": lambda x: get_cam(x[:, :, :, :].to(device), acam)
        }
