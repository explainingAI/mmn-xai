""" Official implementation of RISE (Random Input Sampling for Explanation).

Originally written by: Vitali Petsiuk, 2021.
References: Petsiuk, V., et al. (2018). RISE: Randomized Input Sampling for Explanation.
Link: https://github.com/eclique/RISE
"""
import numpy as np
import torch
...
class RISE(nn.Module):
    ...
    def generate_masks(self, N, s, p1, savepath="masks.npy"):
        ...
        self.masks = self.masks.to(self.__device)
        self.N = N
        self.p1 = p1

    def load_masks(self, filepath):
        self.masks = np.load(filepath)
        self.masks = torch.from_numpy(self.masks).float().to(self.__device)
